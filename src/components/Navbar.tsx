...
function Navbar() {
  ...
  const currentFolderId = openWindows.length > 0 ? openWindows[openWindows.length - 1].id : null;
  const path = useMemo(() => currentFolderId ? getPath(items, currentFolderId) : ["Desktop"], [items, currentFolderId]);
  ...
  return (
    <div className="flex justify-between items-center border-[3px] border-black rounded-sm shadow-[3px_3px_0_0_#000] px-3 py-2 font-mono text-sm select-none bg-[#C0C0C0]">
      <div className="flex gap-4 items-center">
        ...
        <div className="ml-4 text-xs text-black/80">
          {path.join(" / ")}
        </div>
      </div>
